...
int y=n;
    n = n - pref[digits - 2];
    int pos_in_that_digited_num = (n + digits-1) / digits;
    int pos_of_digit_in_the_num = n % digits;

    if (pos_of_digit_in_the_num == 0)
    {
        pos_of_digit_in_the_num = digits;
    }
    int base = power(10, digits - 1);
    int num = base + pos_in_that_digited_num - 1;
    string s = to_string(num);
    // bug(digits,num,base,y,n,s);
    
    cout << s[pos_of_digit_in_the_num - 1] << "\n";
}
